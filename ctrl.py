...
def setup_components():
    ...
    if ioext.is_present():
        ioext.write_register(0x03, 0xfc)
        ioext.write_register(0x01, 0x02)

# Define functions to expose
def read_adc():
    global f2on, f1on
    regval = ioext.read_register(0x01, 1)
    f2on = bytes(a & b for a, b in zip(regval, b'\x01'))
    ...
    try:
        ...
        result.update( {'C eFUSE': f2on == b'\x01' } )
...
def turn_on():
    ...
    if ioext.is_present():
        regval = ioext.read_register(0x01, 1)
        print(f"ioext: @0x01: {regval}")
        trgval = bytes(a | b for a, b in zip(regval, b'\x01'))
...
def fx3_rst():
    if ioext.is_present():
        ...
        print(f"=> {trgval}")
        time.sleep(1)
        trgval = bytes(a | b for a, b in zip(regval, b'\x02'))
        ioext.write_register(0x01, trgval)
        print(f"=> {trgval}")
    return True
